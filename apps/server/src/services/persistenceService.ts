...
export class PersistenceService {
    ...
    try {
      ...
      try {
        ...
        return {
          ...
          bootstrap: {
            backfillCompletedAt: bootstrapRow?.value
              ? bootstrapRow.value
              : null,
          },
      ...
      console.warn(
        `Failed to load llm-pulse state from ${this.statePath}`,
        error,
      );
      return null;
    }
  ...
  async savePulseState(
    ...
    try {
      ...
      database.exec("BEGIN");
      try {
        database.exec("DELETE FROM pulse_logs");
        ...
        upsertState.run(
          "poll_status",
          JSON.stringify(state.pollStatus),
          savedAt,
        );
