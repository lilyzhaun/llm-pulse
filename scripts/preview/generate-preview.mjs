...
const snap = {
  ...
  models: [
    m("gpt-5", "available", 0.99, 0.8, 3200, [
      ...b("available", 52, 5, 18, 1.0, 0.7),
      ...b("available", 8, 3, 10, 0.95, 0.9),
    ]),
    m("claude-sonnet-4.5", "available", 0.97, 1.1, 1800, [
      ...b("available", 48, 4, 12, 0.98, 1.0),
      ...b("degraded", 6, 2, 5, 0.7, 1.8),
      ...b("available", 6, 3, 8, 0.95, 1.1),
    ]),
    m("gemini-2.5-pro", "degraded", 0.82, 2.4, 960, [
      ...b("available", 30, 3, 10, 0.95, 1.2),
      ...b("degraded", 20, 2, 6, 0.6, 2.8),
      ...b("available", 10, 3, 7, 0.9, 1.4),
    ]),
    m("deepseek-r2", "available", 0.94, 1.4, 720, [
      ...b("available", 50, 3, 9, 0.93, 1.4),
      ...b("degraded", 10, 1, 4, 0.7, 2.0),
    ]),
    m("qwen3-max", "unavailable", 0.0, null, 0, [
      ...b("unavailable", 60, 0, 0, 0, 0),
    ]),
    m("llama-4-scout", "available", 0.91, 1.3, 580, [
      ...b("available", 45, 2, 7, 0.91, 1.3),
      ...b("degraded", 15, 1, 3, 0.6, 1.9),
    ]),
  ],
};
...
function m(name, status, sr, lat, total, beats) {
  return {
    modelName: name,
    status,
    successCount: Math.round(total * sr),
    errorCount: Math.round(total * (1 - sr)),
    totalCount: total,
    successRate: sr,
    averageLatencySeconds: lat,
    lastSeenAt: "2026-06-23T07:59:00.000Z",
    tokens: {
      input: Math.round(total * 350),
      cacheInput: Math.round(total * 80),
      output: Math.round(total * 120),
      total: Math.round(total * 550),
    },
    cost: { quota: Math.round(total * 0.15 * 100) / 100 },
    rpm: {
      average: Math.round((total / 60) * 10) / 10,
      peak: Math.round((total / 60) * 35) / 10,
    },
    tpm: {
      average: Math.round((total * 550) / 60),
      peak: Math.round(((total * 550) / 60) * 3),
    },
    heartbeat: {
      healthyBuckets: beats.filter((x) => x.status === "available").length,
      degradedBuckets: beats.filter((x) => x.status === "degraded").length,
      unavailableBuckets: beats.filter((x) => x.status === "unavailable")
        .length,
      unknownBuckets: 0,
      observedBuckets: beats.length,
      availabilityRate:
        beats.filter((x) => x.status === "available").length / beats.length,
...
async function main() {
  ...
  if (!existsSync(resolve(frontendDir, "dist/index.html"))) {
    console.error("请先构建前端");
    process.exit(1);
  }
  ...
  try {
    ...
    const { pg: pL, ctx: cL } = await makePage(browser, "light", {
      width: DESKTOP.w,
      height: DESKTOP.h,
    });
    img.dl = t("dl.png");
    await pL.screenshot({
      path: img.dl,
      clip: { x: 0, y: 0, width: DESKTOP.w, height: DESKTOP.h },
      type: "png",
    });
    ...
    if (ce) {
      ...
      const x = Math.min(Math.max(0, b.x - 10), vw - 1);
      const y = Math.min(Math.max(0, b.y - 10), vh - 1);
      await pL.screenshot({
        path: img.card,
        clip: {
          x,
          y,
          width: Math.max(10, Math.min(vw - x, b.width + 20)),
          height: Math.max(10, Math.min(vh - y, b.height + 20)),
        },
      });
    }
    ...
    if (be) {
      ...
      const x = Math.min(Math.max(0, b.x - 14), vw - 1);
      const y = Math.min(Math.max(0, b.y - 14), vh - 1);
      await pL.screenshot({
        path: img.beat,
        clip: {
          x,
          y,
          width: Math.max(10, Math.min(vw - x, b.width + 28)),
          height: Math.max(10, Math.min(vh - y, b.height + 28)),
        },
      });
    }
    ...
    // Desktop dark
    console.log("桌面 Dark...");
    const { pg: pD, ctx: cD } = await makePage(browser, "dark", {
      width: DESKTOP.w,
      height: DESKTOP.h,
    });
    img.dd = t("dd.png");
    ...
    const cML = await browser.newContext({
      viewport: { width: MOBILE_CSS.w, height: MOBILE_CSS.h },
      deviceScaleFactor: MOBILE_DSF,
      isMobile: true,
      hasTouch: true,
    });
    const pML = await cML.newPage();
    await pML.route("**/status/api/pulse", async (r) => {
      await r.fulfill({
        status: 200,
        contentType: "application/json",
        body: JSON.stringify(snap),
    ...
    console.log("移动 Dark...");
    const cMD = await browser.newContext({
      viewport: { width: MOBILE_CSS.w, height: MOBILE_CSS.h },
      deviceScaleFactor: MOBILE_DSF,
      isMobile: true,
      hasTouch: true,
    ...
    await pMD.route("**/status/api/pulse", async (r) => {
      await r.fulfill({
        status: 200,
        contentType: "application/json",
        body: JSON.stringify(snap),
